...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		else if (type == "Player")
		{
			...
			// set respawn position that was stored in json
			Vector3 pos;
			GetVectorFromJSON(actorValue, "pos", pos);
		...
		// TODO: Add else ifs for other actor types
			...
			LaserMine* mine = new LaserMine(game);
			actor = mine;
		}
		else if (type == "Checkpoint") {
			CheckPoint* cp = new CheckPoint(game);
			// set text displayed when checkpoint reached
			std::string text;
			GetStringFromJSON(actorValue, "text", text);
			cp->setText(text);
			// set the string for next level if this checkpoint leads to another level
			if (GetStringFromJSON(actorValue, "level", text))
				cp->setLevelString(text);
			...
			SecurityCam* cam = new SecurityCam(game);
			actor = cam;
		}
