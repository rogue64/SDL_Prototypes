...
void Game::LoadData() {
	...
	background->SetPosition(Vector2(WINDOW_W / 2, WINDOW_H / 2)); // put in center so it covers window
	SpriteComponent* spr_back = new SpriteComponent(background, 0);
	spr_back->SetTexture(getTexture("Assets/Stars.png"));
	// load player object
	Ship* test = new Ship(this);
	test->SetPosition(Vector2(WINDOW_W / 2, WINDOW_H / 2));
	// initial asteroids
...
void Game::addAsteroid(Asteroid* ast) {
	my_asteroids.push_back(ast);
}

void Game::removeAsteroid(Asteroid* ast) {
	std::vector<Asteroid*>::iterator i = my_asteroids.begin();
	for (; i != my_asteroids.end(); i++)
		if (*i == ast)
			break;
	if (i != my_asteroids.end())
		my_asteroids.erase(i);
}

Asteroid* Game::asteroidCollision(Actor* actor) const {
	// iterate through my_asteroids and see if the given actor is within 70 pixels
	for (size_t i = 0; i < my_asteroids.size(); i++)
		if ((actor->GetPosition() - my_asteroids[i]->GetPosition()).LengthSq() <= 70 * 70)
			return my_asteroids[i];
